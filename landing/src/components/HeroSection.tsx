...
const HEADLINES = [
  "We keep datacenters alive in orbit.",
  "A supervised multi-agent command center for orbital compute.",
  "Compute that never comes down.",
] as const;
...
const VIDEOS = [
  "/videos/hero-orbit-1.mp4",
  "/videos/hero-orbit-2.mp4",
  "/videos/hero-orbit-3.mp4",
] as const;
...
export function HeroSection() {
  ...
  useEffect(() => {
    ...
    let raf = 0;
    let running = true;
    let active = 0; // only the active clip (and the next) is scrubbed, to stay smooth
    ...
    const tick = () => {
      if (!running) return;
      // Only scrub the visible clip and the one about to fade in.
        ...
        <div className="site-container relative z-10 mt-auto flex w-full items-end pb-16 lg:pb-[135px]">
          <div className="w-full max-w-[1100px]">
            {HEADLINES.map((line, i) => (
              <h1
                key={line}
                className={`font-normal leading-[0.95] tracking-[-0.045em] text-white text-[clamp(2.5rem,7vw,4.375rem)] ${
                  i === segment
