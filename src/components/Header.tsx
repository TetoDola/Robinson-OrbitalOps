...
import { TerminalLogo, ChevronDownIcon, PhoneIcon } from "@/components/icons";

const NAV_ITEMS = [
  { label: "System", hasDropdown: true },
  { label: "Markets", hasDropdown: true },
  { label: "Featured", hasDropdown: true },
  ...
  return (
    <>
      <header className="pointer-events-none fixed inset-x-0 top-[46px] z-50 text-white">
        <div className="site-container">
          <div className="pointer-events-auto mx-auto flex h-[78px] w-full max-w-[944px] items-center justify-between gap-6 rounded-lg bg-black/30 px-6 py-[18px] backdrop-blur-[30px]">
            <a href="/" aria-label="Terminal — home" className="shrink-0">
              <TerminalLogo className="h-[26px] w-auto text-white transition-colors hover:text-[var(--c-lime)]" />
            </a>
      ...
      <div className={`fixed inset-0 z-40 lg:hidden ${mobileOpen ? "pointer-events-auto" : "pointer-events-none"}`} aria-hidden={!mobileOpen}>
        ...
        >
          <div className="flex min-h-[78px] items-center border-b border-white/15 px-6">
